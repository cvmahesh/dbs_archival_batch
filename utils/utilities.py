...
def get_folder_name_from_argument_date(date_string):
    ...
    try:
        ...
        logger.error(f"Error: Invalid date format. Use 'YYYY-MM-DD'. {e}")
        return None
...
if __name__ == "__main__":
    ...
    logger.info("Folder name from system date:", get_folder_name_from_system_date())

    # Get folder name based on argument date
    sample_date = "2024-12-28"
    logger.info("Folder name from argument date:", get_folder_name_from_argument_date(sample_date))

    # Invalid date example
    invalid_date = "28-12-2024"
    logger.info("Invalid date example:", get_folder_name_from_argument_date(invalid_date))


    path_separator = get_path_separator()
    logger.info(f"Path separator for the current OS: '{path_separator}'")
